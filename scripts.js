...
function showSolution(arr){
	for(let i = 0; i < arr.length; i++){
		for(let j = 0; j < arr[i].length; j++){
			...
			console.log(arr[i]);
			console.log(arr[i][j]);
			console.log(arr[i][j].charAt(0));
			console.log(arr[i][j].charAt(2));
...
$(document).ready(() => {
	...
	let counter = 0;
	piecesArr = loadNextPuzzle(fenArr[counter]);
	...
	$("#show-solution-button").click( ()=>{
		...
		removeSolution();
		if(showSolutionFlag)
			showSolution(piecesArr);
	});

	/* on chessboard resize, reload the solution*/
	$("#chessboard").resize(()=>{
		removeSolution();
		if(showSolutionFlag)
			showSolution(piecesArr);
